...
final class HomeViewModel {
    ...
    final class State {
        struct CollectionViewModels {
            var bannerItems: [HomeItem]?
            var horizontalProductItems: [HomeItem]?
            var couponItems: [HomeItem]?
            var verticalProductItems: [HomeItem]?
            var themeItems: (title: String, item: [HomeItem])?
        }
...
private extension HomeViewModel {
    ...
    func makeItems(response: HomeResponse) async {
        state.collectionViewModels.bannerItems = response.banners.map { HomeItem.banner($0) }
        state.collectionViewModels.horizontalProductItems = response.horizontalProducts.map { HomeItem.horizontalProduct($0) }
        state.collectionViewModels.verticalProductItems = response.verticalProducts.map { HomeItem.verticalProduct($0) }
        state.collectionViewModels.themeItems = ("테마관" , response.themes.map { HomeItem.theme($0) })
    }
    
    @MainActor
    func transformCoupon(isEnabled: Bool) async {
        state.collectionViewModels.couponItems = [HomeItem.coupon(CouponInfo(state: isEnabled ? .disable : .enable))]
    }
}
